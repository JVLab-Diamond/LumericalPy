...
def mode_profile_plot_xy(filename, objname, resultname, display):
...
def monitor_profile_plot_xz(filename, objname, resultname, savename, display):
    ...
    for i,j in zip(wave, val):
        val_array = np.sqrt(abs(j[0])**2+abs(j[1])**2+abs(j[2])**2)
        plt.imshow(val_array, origin='lower', extent=[x_min, x_max, y_min, y_max], aspect=8, cmap='inferno')
        # plt.colorbar()
        # plt.title(f'{resultname} {int(i*1e9)}nm')
        plt.ylabel('um')
        plt.xlabel('um')
        plt.savefig(savename+f'_{objname}_{resultname}_{int(i*1e9)}.png', dpi=300, transparent=True, bbox_inches='tight')
        if display == 1:
            plt.show()
        plt.close()
...
def farfield3d_plot(filename, objname, save_name, slice_num=None):
    ...
    radius_list = np.asarray([np.sqrt(i**2+j**2) for i,j in zip(cartesian_x, cartesian_y)])
    theta_list = []
    ...
    if slice_num==None:
        for i in range(len(data['wavelength'])):
            ...
            plt.tricontourf([i+np.pi/2 for i in theta_list.flatten()], radius_list.flatten(), E_plot.flatten(), levels=256, cmap='jet')
            plt.grid(alpha=0.9)
...
def overlap_integral(file1, file2, slice_num1, slice_num2, display):
    ...
    if display == 1:
        ...
        plt.subplot(projection="polar")
        plt.ylim(0, max(ux))
        plt.tricontourf([i+np.pi/2 for i in theta_list.flatten()], radius_list.flatten(), E1_plot.flatten(), levels=256, cmap='jet', alpha=0.5)
        plt.tricontourf([i+np.pi/2 for i in theta_list.flatten()], radius_list.flatten(), E2_plot.flatten(), levels=256, cmap='jet', alpha=0.3)
        plt.grid(alpha=0.9)
        plt.show()
